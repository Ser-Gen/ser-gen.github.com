...
function drawGame()
{
	...
	obj.drawGameStart = function(){
		...
		if(!obj.isAdmin && (hours >= 9 && hours < 18)) {
			alert('Время работать. Игра доступна до 9:00 и после 18:00');
		} else if(!obj.bDraw) {
		...
		$.ajax({
			...
			success: function(result) {
				if (result != undefined && result.length > 0) {
					$('.screen-start .result ul').html(result);
				}
				if (r % 10 == 0 || r % 10 >= 5
					|| (r % 100 >= 11 && r % 100 <= 14)) {
...
function drawLife()
{
	...
	obj.show = function() {
		if(obj.bShow) {
			...
			ctx.fillText(obj.symb + obj.drawLife, obj.X + obj.width, obj.Y + 27);
			cGame.pushShow(obj.show);
		}
	}
	obj.damage = function() {
