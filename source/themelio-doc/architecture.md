...
Элементы интерфейса на страницах сайта редко бывают **абстрактными**, они связаны общим назначением в более общих блоках и их обьединениях. Внутри таких блоков формируется **контекст**.

Каждому элементу интерфейса необходим _один точный класс_, обозначающий его принадлежность к его **контексту**. При этом такой _контекстный элемент_ должен наследовать все необходимые свойства **абстрактных блоков**, которые ему необходимы для выполнения своих функций.
...
Первый способ проблемен тем, что переносит оформление в разметку, чем затрудняет изменяемость блоков.
...
Однако, стоит помнить, что объединияя абстрактные блоки в контекстном, можно затруднить их взаимодействие, особенно если производить его при помощи составных селекторов. Контекстные блоки **должны упрощать** модификацию абстрактных, но **не должны усложнять** работу с ними.

Заменять класс у элементов блока не нужно, их модификация должна производиться от контекстного блока.
