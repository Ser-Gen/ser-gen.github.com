...
var pos = {
    ...
    grad = d.getElementById('OverlayGradientMain'),
...
weatherNow.addEventListener(actionEventUp, function () {
  changeOverlayState(1);
}, false);
weatherForecastHours.addEventListener(actionEventUp, function () {
  changeOverlayState(2);
}, false);
weatherForecastDaily.addEventListener(actionEventUp, function () {
  changeOverlayState(0);
}, false);
...
function documentKeyHandler(e) {
  ...
  if ((e.keyCode == 32) && !addressIsEdited) {
    e.preventDefault();
    switch (overlayState) {
      case 0:
  ...
  if ((e.shiftKey && (((e.keyCode > 47) && (e.keyCode < 91)) || (e.keyCode == 188) || (e.keyCode == 190) || (e.keyCode == 192) || (e.keyCode == 219) || (e.keyCode == 221) || (e.keyCode == 222))) && !addressIsEdited) {
    addressFocus();
  }
}
...
function findName(pos) {
  ...
  geocoder.geocode({'latLng': latlng}, function(results, status) {
    if (status == google.maps.GeocoderStatus.OK) {
      if (results) {
        address.value = setAddressName(results);
      } else {
        address.value = 'Город не нашёлся :(';
      }
    } else {
...
function locator(p) {
  if ((p === undefined) || (pos.alternate === false)) {
...
function insertForecastHours(forecastHours) {
...
function insertForecastDaily(forecastDaily) {
  // http://stackoverflow.com/questions/3818193/how-to-add-number-of-days-to-todays-date
  var date = new Date();
